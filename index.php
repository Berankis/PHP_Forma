	...
	<body>
		...
		<div class='container w-75 mt-4'>
			<form>
				...
				<div class="form-group">
					...
					<textarea type="text" class="form-control" maxlength="2554" id="comment" aria-describeby='comment_validation' placeholder="Enter comment"></textarea>
		...
		<?php
			...
			while($data = mysqli_fetch_row($query)){
				$id = $data[0];
				$name = $data[2];
				$email = $data[1];//addslashes(htmlspecialchars(trim(preg_replace('/\s\s+/', ' ', $data[3])),ENT_QUOTES));
				$text = str_replace(array("\r\n", "\n", "\r"), ' ', htmlspecialchars(htmlentities(addslashes($data[3]))));
				$date = $data[4];
